# -*- coding: utf-8 -*-
"""S_&_G_Deep_Learning_model.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1osNmLoHq9tA83cDwIdlBeSk0NuCiJXjs
"""
...
def predict_sequence(input_sequence):
    ...
    sequence = tokenizer_input.texts_to_sequences([input_sequence])
    padded_sequence = pad_sequences(sequence, maxlen=max_input_length, padding='post')

    # Make the prediction
    prediction = model.predict(padded_sequence)

    # Get the predicted sequence (argmax to find the most likely word at each time step)
    predicted_sequence = np.argmax(prediction[0], axis=-1)

    # Convert predicted sequence back to text
    predicted_text = tokenizer_target.sequences_to_texts([predicted_sequence])
    return predicted_text[0]

# Function to take user input and get the corrected output
def get_corrected_output(user_input):
    corrected_output = predict_sequence(user_input)
    return corrected_output

# Example usage: Get corrected output from user input
user_input = input("Enter a sentence to correct: ")
corrected_output = get_corrected_output(user_input)
...
# Main loop to continuously take user input and provide corrected output
while True:
    user_input = input("\nEnter a sentence to correct (or type 'exit' to quit): ")
...
# Function to calculate accuracy between two paragraphs
def calculate_accuracy(true_paragraph, predicted_paragraph):
    true_words = true_paragraph.split()
    predicted_words = predicted_paragraph.split()

    # Calculate the number of correctly predicted words
    correct_count = sum(1 for t, p in zip(true_words, predicted_words) if t == p)

    # Accuracy calculation
    accuracy = correct_count / max(len(true_words), 1) * 100
    return accuracy

# Main loop to continuously take user input, provide corrected output, and calculate accuracy
while True:
    user_input = input("\nEnter a sentence to correct (or type 'exit' to quit): ")
    if user_input.lower() == "exit":
        print("\nExiting the program...")
    ...
    # Get the corrected output from the model
    corrected_output = get_corrected_output(user_input)

    # Calculate accuracy
    accuracy = calculate_accuracy(true_paragraph, corrected_output)

    # Print the results
    print("\nCorrected Output:", corrected_output)
    print("True Paragraph:", true_paragraph)
